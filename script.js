...
function privateKeyFromPem(pem) {
    ...
    let obj = forge.asn1.fromDer(msg.body);
    // get PrivateKeyInfo
    let privateKeyInfo = {}
    let errors = [];
    ...
    privateKeyInfo.privateKeyVersion = forge.util.bytesToHex(privateKeyInfo.privateKeyVersion);
    ...
    ecPrivateKeyInfo.privateKey = forge.util.bytesToHex(ecPrivateKeyInfo.privateKey);
    if ('publicKey' in ecPrivateKeyInfo) {
        ecPrivateKeyInfo.publicKey = forge.util.bytesToHex(ecPrivateKeyInfo.publicKey);
    }
    if ('namedCurveOid' in ecPrivateKeyInfo) {
        ecPrivateKeyInfo.namedCurveOid = asn1.derToOid(ecPrivateKeyInfo.namedCurveOid);
    }
...
if (enableShangMi == "true") {
    ...
    const publicKey = ('publicKey' in privateKeyInfo.ecPrivateKey)
        ? privateKeyInfo.ecPrivateKey.publicKey
        : publicKeyFromPem(publicKeyPem).publicKey;

    const sm2 = new SM2Lib.SM2;
    const utils = SM2Lib.utils;
    const messageByteArray = utils.stringToByteArrayInUtf8(message);
    const idByteArray = utils.stringToByteArrayInUtf8('1234567812345678');
    const signHex = sm2.sign(publicKey, privateKeyInfo.ecPrivateKey.privateKey, messageByteArray, idByteArray);
    const signature = utils.byteArrayToBase64(utils.hexToByteArray(signHex));
